...
directory with HistTupleProducerTask output produced using
histTuple_flavor: DNN_training (config/global.yaml; local mount or EOS path)
-- the local repo mirror is empty, so this has not been run end-to-end
...
def main():
    ...
    parser.add_argument(
        ...
        help="one or more eras to train on (default: config.ERAS, i.e. all configured eras). "
             "Not every era has histTuples produced at all times -- use this to restrict to "
    ...
    if not shards:
        raise RuntimeError(f"No histTuple shards found under {cfg.data.data_root}; check --data-root and config.")
