...
from Analysis.dnn_kinematics import convert_to_numpy
...
# Branches read from the HistTuple produced by HistTupleProducerTask with
# histTuple_flavor: DNN_training (config/global.yaml) -- that flavor's
# fullResolution_variables: list is exactly this list (plus the
# deep-tau-version-dependent branches added by raw_branches() below), saved
# unbinned/full-precision by HistTupleProducer.py. NNInterface.array_inputs
# (interface.py) documents the feature set consumed at inference time.
#
# Unlike a raw AnaTuple, the HistTuple already has
# "SelectedFatJet_{pt,eta,phi,mass}_boosted" and "boosted_baseline" as flat,
# per-event branches -- hh_bbtautau.py::defineBoostedVariables() (RDataFrame,
# FLAF's RemoveOverlaps/ReorderObjects C++ helpers) runs upstream inside
# HistTupleProducerTask, so this loader no longer needs to reproduce that
# selection itself from jagged SelectedFatJet_* arrays (see git history for
# the old AnaTuple-reading version that did).
RAW_BRANCHES = [
    "event", "channelId",
    ...
    "SelectedFatJet_phi_boosted", "SelectedFatJet_mass_boosted", "boosted_baseline",
...
def histtuple_files(data_root: str, era: str, dataset: str) -> list[str]:
    """Locate histTuple ROOT files for one (era, dataset) under
    {data_root}/HistTuples/{era}/{dataset}/*.root -- HistTupleProducerTask's
    output layout (FLAF/Analysis/tasks.py HistTupleProducerTask.output():
    "{version}/HistTuples/{period}/{dataset}/histTuple_*.root"), mirroring the
    "{version}/AnaTuples/{period}/{dataset}/anaTuple_*.root" layout of its
    AnaTupleMergeTask input. *data_root* is expected to include the version
    segment (e.g. ".../v2608_dnn"). Supports both local filesystem paths
    (glob) and root://host//path XRootD URLs (remote directory listing),
    which is how the davs://cmseos.fnal.gov:9000/... production storage in
    ...
    dir_path = os.path.join(data_root, "HistTuples", era, dataset)
...
def build_features(array, period: str) -> tuple[dict[str, np.ndarray], np.ndarray, np.ndarray]:
    """Turns a selected array chunk (already carrying the HistTuple's flat
    "SelectedFatJet_*_boosted"/"boosted_baseline" branches -- no jagged
    FatJet-selection logic needed here, see the RAW_BRANCHES module docstring)
    into (cat_inputs, lbn_vectors, extra_continuous), reusing
    dnn_kinematics.convert_to_numpy() (ROOT/TF-free, shared with
    Analysis/DNN_application.py's inference path) for the raw four-vector
    construction, then applying the same rotation/composite/masking steps
    NNInterface.predict() applies at inference time (reimplemented above in
    _apply_rotation_and_composites, so interface.py stays untouched).

    convert_to_numpy() also returns a "pair_type" field using its own
    hardcoded pairtype_map={23:0,13:1,33:2} (matching DNN_application.py's
    inference-time convention); that
...
def load_shard(
    ...
    files = histtuple_files(cfg.data_root, era, dataset)
    if not files:
        return None
...
if __name__ == "__main__":
    # Synthetic smoke test for select_training_events -- no real histTuple needed.
    n = 9
    fake = {
